...
from gbn_client import GBN_Client
from sr_client import SR_Client
...
class RDT_Client:
    ...
    def __init__(self, server_address, mode='GBN', send_fail_delay=1, max_payload_size=100, sock=None, timeout_value=5, window_size=10, initial_sequence_number=0, sequence_number_count=None, corrupt_probability=0):
        ...
        if mode == 'GBN':
            self.client = GBN_Client(sock = sock, server_address = server_address, timeout_delay = timeout_value, 
                    window_size = window_size, initial_sequence_number = initial_sequence_number, sequence_number_count = sequence_number_count, corrupt_probability = corrupt_probability)
        elif mode == 'SR':
    ...
    def send_message(self, message):
        ...
        self.send_message_set([message[0+i:self.MAX_PAYLOAD_SIZE+i] for i in range(0, len(message), self.MAX_PAYLOAD_SIZE)])
